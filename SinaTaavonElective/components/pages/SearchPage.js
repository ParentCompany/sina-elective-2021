...
class SearchPage extends Component {
	...
	componentDidMount = async () => {
		...
		if (
			...
			navigation.navigate('LoginPage', { screen: 'SearchPage' })
		} else if (
			token !== null ||
			token !== undefined ||
			token !== '' ||
				...
				<ScrollView>
					...
					<View style={styles.reviewRow}>
						...
						<RadioButton
							...
							status={
								checked === 'reviewed' ? 'checked' : 'unchecked'
							}
						...
						<RadioButton
							...
							status={
								checked === 'favourite' ? 'checked' : 'unchecked'
							}
					...
					{shop?.map((shop, index) => (
						<Card key={index} style={styles.spaceCard}>
							...
							/>
							<Card.Cover
								source={{ uri: coverPhoto[index + offset].path }}
							/>
							<Card.Actions>
								<Button
									mode='text'
